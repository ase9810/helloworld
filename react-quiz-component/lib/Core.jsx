...
class Core extends Component {
  constructor(props) {
    ...
    this.state = {
      ...
      lastCheck: this.props.lastCheck != undefined ? this.props.lastCheck : false,
    };
  }

  checkAnswer = (index, correctAnswer, answerSelectionType) => {
    const { correct, incorrect, currentQuestionIndex, continueTillCorrect, userInput, lastCheck } = this.state;
    ...
    if (lastCheck) {
      if (answerSelectionType == 'single') {
        ...
        if (index == correctAnswer) {
          ...
          else if (incorrect.indexOf(currentQuestionIndex) > 0 && correct.indexOf(currentQuestionIndex) < 0) {
            incorrect.pop(currentQuestionIndex);
            correct.push(currentQuestionIndex);
          }
          let classNameAll = {
            0: { className: "" },
          ...
          } else if (correct.indexOf(currentQuestionIndex) > 0 && incorrect.indexOf(currentQuestionIndex) < 0) {
            correct.pop(currentQuestionIndex);
            incorrect.push(currentQuestionIndex);
          }
          let classNameAll = {
            0: { className: "" },
        ...
        if (maxNumberOfMultipleSelection == userAttempt) {
          ...
          this.setState({
            userInput,
            userAttempt: userAttempt < maxNumberOfMultipleSelection ? userAttempt + 1 : userAttempt
          })
        }
      ...
      return (
        <div key={index}>
          <button disabled={true} className={"answerBtn btn " + answerBtnCorrectClassName + answerBtnIncorrectClassName}>
            {questionType == 'text' && <span><MathJax.Text text={answer} /></span>}
    ...
    return questions.map((question, index) => {
      ...
      return (
        <div className="result-answer-wrapper" key={index + 1}>
          ...
          <h3><MathJax.Text text={`Q${question.questionIndex}: ${question.question}`} /></h3>
          {
  ...
  renderAnswers = (question, buttons) => {
    ...
    return answers.map((answer, index) => {
      if (buttons[index] != undefined) {
        return (
          <button key={index} disabled={buttons[index].disabled || false}
        ...
        return (
          <button key={index}
            ...
            {questionType == 'text' && <MathJax.Text text={answer} />}
            {questionType == 'photo' && <img src={answer} />}
          </button>
        )
      }
      ...
      <div className="questionWrapper">
        {!endQuiz &&
          <div className="questionWrapperBody">
            ...
            <h3><MathJax.Text text={question.question} /></h3>
            {
              this.renderTags(answerSelectionType, question.correctAnswer.length)
            }
            {
