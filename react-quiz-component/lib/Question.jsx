  ...
  renderQuizResultQuestions = () => {
    ...
    return questions.map((question, index) => {
      ...
      return (
        ...
        <h3 dangerouslySetInnerHTML={`Q${question.questionIndex}: ${<MathJax.Text text={this.rawMarkup(question.question)}/>}`}/> 
                ...
                return(
                  <div key={index}>
                     <button disabled={true} className={"answerBtn btn" + (index+1 == question.correctAnswer ? ' correct ': '') + (userInputIndex != question.correctAnswer && index+1 == userInputIndex ? ' incorrect ' : '')}>
                      { question.questionType == 'text' && <span><MathJax.Text text={answer}/></span> }
      ...
      <div className="questionWrapper">
        {!this.state.endQuiz &&
          <div className="questionWrapperBody">
            ...
            <h3 dangerouslySetInnerHTML={<MathJax.Text text={this.rawMarkup(question.question)}/>}/> 
            {
              question.answers.map( (answer, index) => {
                if(this.state.buttons[index] != undefined) {
                  ...
                  return (
                    ...
                    { question.questionType == 'text' && <MathJax.Text text={answer}/> }
                    { question.questionType == 'photo' && <img src={answer}/> }
                    </button>
                  )
                }
